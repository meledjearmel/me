...
return [
    ...
    'info' => [
         ...
         */
        'description' => <<<'MD'
API de gestion du portfolio, destinée à l'application mobile.

**Authentification** : `POST /v1/auth/login` renvoie un jeton à envoyer ensuite dans l'en-tête
`Authorization: Bearer {token}`.

**Réponses** : les ressources ne sont pas enveloppées dans `data`, sauf les listes paginées
(`data`, `links`, `meta`). Les textes traduits sont renvoyés dans toutes les langues (`{"fr": "...", "en": "..."}`).

**Fichiers** : pour envoyer une image en modification, faire un `POST` en `multipart/form-data`
avec le champ `_method` (`PUT` ou `PATCH`).
MD,
    ],
